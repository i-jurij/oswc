    ...
    <div>
        <?php
            ...
            if (!empty($users) && is_array($users) && !empty($check) && is_string($check)) {
                echo ' <form action="" method="post" id="'.$form.'" class="pad form_del_ch">
        ...
        if (!empty($data['res'])) {
            ...
            if (is_array($data['res'])) {
                ...
                echo '<div class="'.$pad.'">'.$data['res'].'</div>';
            }
        }
        ?>
    </div>
